...
import { Container, Button, ListGroup, Alert, Spinner, Modal } from 'react-bootstrap';
...
const EnderecosCliente = () => {
    ...
    const loadAddresses = useCallback(async () => {
        ...
        if (result.success && result.data?.enderecos) setAddresses(result.data.enderecos);
        else {
            setAddresses([]);
            if (!result.success) {
                setError(true);
    ...
    const handleEditClick = (address) => { setEditingAddress(address); setShowModal(true); };
    const handleModalClose = () => { setShowModal(false); setEditingAddress(null); };

    const handleDeleteConfirmation = (address) => { setAddressToDelete(address); setShowDeleteModal(true); };
    const handleDeleteClose = () => { setShowDeleteModal(false); setAddressToDelete(null); };
        ...
        <Container style={{ maxWidth: '800px' }} className="mt-5 mb-5 p-4 border rounded shadow-sm">
            ...
            {addresses.length === 0 ? (
                <Alert variant="info" className="text-center">
                    Você ainda não tem nenhum endereço cadastrado. Use o botão acima para incluir o primeiro!
                </Alert>
            ) : (
                    ...
                    {addresses.map(addr => (
                        <ListGroup.Item key={addr.id} className="d-flex justify-content-between align-items-center">
                            ...
                            <div className="d-flex gap-2">
                                <Button variant="outline-secondary" size="sm" onClick={() => handleEditClick(addr)}>
                                    <i className="bi bi-pencil-square"></i> Alterar
                                </Button>
                                <Button variant="outline-danger" size="sm" onClick={() => handleDeleteConfirmation(addr)}>
            ...
            <Modal show={showModal} onHide={handleModalClose} centered>
                <Modal.Header closeButton>
                    <Modal.Title>{editingAddress ? 'Editar Endereço' : 'Adicionar Novo Endereço'}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <AddressForm addressData={editingAddress} onSuccess={loadAddresses} onCancel={handleModalClose} />
                </Modal.Body>
            </Modal>
            ...
            <Modal show={showDeleteModal} onHide={handleDeleteClose} centered>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="danger" onClick={handleConfirmDelete}><i className="bi bi-trash me-2"></i> Confirmar Exclusão</Button>
                </Modal.Footer>
            </Modal>
        </Container>
    );
