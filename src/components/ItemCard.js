...
const ItemCard = ({ item }) => {
    const { cartState, dispatch, addToCartWithStock } = useCart();
    ...
    const handleAddToCart = async () => {
        ...
        try {
            // Buscar quantidade atual do item no carrinho do backend
            ...
            if (currentQty + quantity > item.estoque) {
                showAlert({title:'Aviso!',message:'O produto selecionado não tem mais estoque disponível',type:'sucsess',duration:5000, bg:'#0d6efd'});
                setIsLoading(false);
                return;
            }
            ...
            setFeedback({ type: result.success ? 'success' : 'danger', message: result.message });
    ...
    // quantidade atual no carrinho para desabilitar botão
                ...
                <Modal.Body>
                    ...
                    <div className="d-flex flex-column flex-md-row">
                        ...
                        <div>
                            ...
                            <p className="mt-2 md-0">
                                <b>Estoque disponível:</b> {item.estoque !== undefined ? item.estoque : 'Indisponível'}
                            </p>
                        </div>
                    </div>
                </Modal.Body>
